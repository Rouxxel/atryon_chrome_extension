...
ALLOWED_POLLING_HOSTS = set(
    data_loader["image_ai_providers"]["black_forest"]["allowed_polling_hosts"]
)
...
def _get_bfl_headers() -> dict:
    ...
    if not api_key:
        raise HTTPException(
            status_code=503, detail="Black Forest API key not configured."
        )
...
async def polling_requests(
    request: Request,
    polling_url: str = Query(
        ..., description="Polling URL returned from the submit MIC endpoint"
    ),
):
    """
    ...
    for attempt in range(1, POLLING_RETRY_ATTEMPTS + 1):
        try:
            ...
            if attempt < POLLING_RETRY_ATTEMPTS:
                log_handler.warning(
                    "[polling_requests] Retrying BFL poll in %.1fs (attempt %s/%s)",
                    POLLING_RETRY_DELAY_SEC,
                    attempt + 1,
                ...
                continue
            raise HTTPException(
                status_code=502, detail="Failed to reach Black Forest API."
            )
