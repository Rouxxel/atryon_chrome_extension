...
def _build_full_prompt(user_prompt: str) -> str:
    ...
    prefix = BF_CFG.get("default_prompt_prefix") or ""
...
async def submit_mic(request: Request, body: SubmitMicBody):
    ...
    url = f"{base_url.rstrip('/')}/{BF_CFG['model_name']}"
    payload = {
        "prompt": full_prompt,
        ...
        "width": BF_CFG.get("width", 1024),
        "height": BF_CFG.get("height", 1024),
        "safety_tolerance": BF_CFG.get("safety_tolerance", 2),
        "output_format": BF_CFG.get("output_format", "jpeg"),
    }
